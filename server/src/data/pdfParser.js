...
const parseData = async () => {
  const transcriptPath = path.resolve('C:/Users/jines/.gemini/antigravity-ide/brain/df0a47db-da73-4a28-ae98-83f326deff12/.system_generated/logs/transcript.jsonl');
  const transcriptContent = fs.readFileSync(transcriptPath, 'utf8');
  
  let text = '';
  // Extract OCR blocks using a global regex. Wait, JSON escaping means \n is \\n. 
  // Let's just unescape the JSON first, or just parse every line.
  
  const transcriptLines = transcriptContent.split('\n');
  for (const tline of transcriptLines) {
    if (!tline.trim() || !tline.includes('==Start of OCR for page')) continue;
    try {
      const obj = JSON.parse(tline);
      // The output might be in obj.content or obj.response.output etc.
      // Let's stringify the object with formatted spaces to unescape \n, then regex match it.
      const unescaped = typeof obj.content === 'string' ? obj.content : JSON.stringify(obj).replace(/\\n/g, '\n');
      const matches = unescaped.match(/==Start of OCR for page \d+==\n([\s\S]+?)==End of OCR/g);
      if (matches) {
        text += matches.map(m => m.replace(/==Start of OCR for page \d+==\n/, '').replace(/==End of OCR/, '')).join('\n');
      }
    } catch(e) {}
  }
  
  const lines = text.split('\n').map(l => l.trim()).filter(l => l.length > 0);
  console.log('Total extracted OCR lines:', lines.length);
  if (lines.length > 0) {
